...
function update(iter::HagerZhangLineSearchIterator, a::LineSearchPoint, b::LineSearchPoint, αc)
    ...
    iter.parameters.verbosity > 2 &&
        @printf("@INFO: Linesearch update: try c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e\n", c.α, c.dϕ, c.ϕ - ϕ₀)
    ...
    while true
        if (b.α - a.α) <= eps(max(b.α, one(b.α)))
            if iter.parameters.verbosity > 0
                @printf("@WARN: Linesearch bisection failure: [a, b] = [%.2e, %.2e], b-a = %.2e, dϕᵃ = %.2e, dϕᵇ = %.2e, (ϕᵇ - ϕᵃ)/(b-a) = %.2e\n", a.α, b.α, b.α - a.α, a.dϕ, b.dϕ, (b.ϕ - a.ϕ)/(b.α - a.α))
            end
            return a, b, numfg
        end
        αc = (1 - θ) * a.α + θ * b.α
...
function bracket(iter::HagerZhangLineSearchIterator{T}, c::LineSearchPoint) where {T}
    ...
    fmax = a.f + ϵ
    iter.parameters.verbosity > 2 &&
        @printf("@INFO: Linesearch start: dϕ₀ = %.2e, ϕ₀ = %.2e\n", a.dϕ, a.ϕ)
    α = c.α
    while true
        ...
        if iter.parameters.verbosity > 2
            @printf("@INFO: Linesearch bracket: try c = %.2e, dϕᶜ = %.2e, ϕᶜ - ϕ₀ = %.2e, wolfe = %d, approxwolfe = %d\n", c.α, c.dϕ, c.ϕ - p₀.ϕ, checkexactwolfe(c, p₀, c₁, c₂), checkapproxwolfe(c, p₀, c₁, c₂, ϵ))
        end
...
function (ls::HagerZhangLineSearch)(fg, x₀, η₀, fg₀ = fg(x₀);
    ...
    while true
        ...
        if done
            ls.verbosity >= 1 &&
                @printf("@INFO: Linesearch converged after %2d iterations: α = %.2e, dϕ = %.2e, ϕ - ϕ₀ = %.2e\n", k, α, dϕ, f - f₀)
            return x, f, g, ξ, α, numfg
        elseif k == ls.maxiter
            ls.verbosity >= 1 &&
                @printf("@INFO: Linesearch not converged after %2d iterations: α = %.2e, dϕ = %.2e, ϕ - ϕ₀ = %.2e\n", k, α, dϕ, f - f₀)
            return x, f, g, ξ, α, numfg
        else
            ls.verbosity >= 2 &&
                @printf("@INFO: Linesearch step %d: [a,b] = [%.2e, %.2e], dϕᵃ = %.2e, dϕᵇ = %.2e, ϕᵃ - ϕ₀ = %.2e, ϕᵇ - ϕ₀ = %.2e\n", k, a.α, b.α, a.dϕ, b.dϕ, a.ϕ - f₀, b.ϕ - f₀)
